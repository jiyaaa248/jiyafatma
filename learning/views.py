...
def submit_quiz(request, quiz_id):
    ...
    # ACHIEVEMENT / BADGES
    # ✅ Unlock next chapter only if user got at least 2 questions correct
    chapter_unlocked = False
    if correct_count >= 2:
        chapter = quiz.chapter
        # Mark chapter as completed to unlock next chapter
        UserChapterCompletion.objects.get_or_create(user=request.user, chapter=chapter)
        chapter_unlocked = True

        # Award chapter completion badge only if all answers are correct
    ...
    # ✅ "Top learner" badge — only awarded to the single user with the highest XP
    top_profile = UserProfile.objects.order_by('-xp').first()
    if top_profile and top_profile.user == request.user:
        award_badge(request.user, "Top learner")

    # ✅ "Supreme warrior" badge — only if user completed ALL chapters AND has highest XP
    total_chapters = Chapter.objects.count()
    user_completed_chapters = UserChapterCompletion.objects.filter(user=request.user).count()
    if user_completed_chapters >= total_chapters and total_chapters > 0:
        top_profile_supreme = UserProfile.objects.order_by('-xp').first()
        if top_profile_supreme and top_profile_supreme.user == request.user:
    ...
    return render(request, "learning/quiz_result.html", {
        ...
        "xp": xp_earned,
        "level": profile.level,
        "chapter_unlocked": chapter_unlocked,  # pass to template if needed
    })
